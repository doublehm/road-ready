...
- [x] Task: Audit `app/static/maps.js` for inefficient re-renders or multiple `setView` calls.
- [x] Task: Implement global `resizeObserver` or improved `invalidateSize` logic for all map containers.
- [x] Task: Optimize Tile Layer settings (fadeAnimation, zoomAnimation, updateWhenIdle, updateWhenZooming).
